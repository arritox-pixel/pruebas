...
def digest_hex(algoritmo, texto):
    if algoritmo == "SHA512":
        obj = hashes.Hash(hashes.SHA512())
    elif algoritmo == "SHA256":
        obj = hashes.Hash(hashes.SHA256())
    elif algoritmo == "MD5":
        obj = hashes.Hash(hashes.MD5())
    else:
        return None

    obj.update(str(texto).encode("utf-8"))
    return obj.finalize().hex()
...
class Ship:
    ...
    def GenID(self):
        ...
        resultado = digest_hex(algoritmo, texto)
...
class Port:
    ...
    def _generar_id_puerto(self, nombre_pais, algoritmo):
        resultado = digest_hex(algoritmo, str(nombre_pais).upper())
        if resultado is None:
            raise ValueError("Algoritmo no soportado")
        return resultado
        ...
        for algoritmo in diccionario:
            ...
            for pais in paises:
                ...
                while numero <= 99:
                    ...
                    calculado = digest_hex(algoritmo, id_claro)
                    if calculado is not None and calculado == ship_hash:
                        return id_claro
                    numero += 1
